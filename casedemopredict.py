...
rus = RandomUnderSampler(sampling_strategy={0: 74760 }, random_state=0)
...
Predict_proba = Logit.predict(X_test)     # n行k列的数组，第i行第j列上的数值是模型预测第i个预测样本的标签为j的概率
print(Predict[0:20])
print(Predict_proba[:])
...
Predict_proba = tree.predict(X_test)
print(Predict_proba[:])
...
Predict_proba = forest.predict(X_test)
print(Predict_proba[:])
...
df_test = df_test.fillna(method='bfill')    # 纵向向前填充
...
Predict = Logit.predict(X1)
df_test["Logit_prob"] = Predict
Predict[0:10]


Predict = tree.predict(X1)
df_test["Tree_prob"] = Predict
Predict[0:10]

Predict = forest.predict(X1)
df_test["Forest_prob"] = Predict
Predict[0:10]
...
print(res.head(10))
res.to_csv(path_or_buf = r"d:\pythondata\prediction5.csv",index = False)
